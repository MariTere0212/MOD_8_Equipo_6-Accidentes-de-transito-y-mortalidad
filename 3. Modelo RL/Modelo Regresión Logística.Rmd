...
```{r}
load("C:/Users/marit/Downloads/datos_minimos_accidentes_limpio.RData")
...
```{r}
# Probabilidad por grupo de edad
prob_edad <- dt_modelo %>%
  group_by(grupo_edad) %>%
...
```{r}
# Probabilidad por tipo de vehículo
prob_veh <- dt_modelo %>%
...
```{r}
# Probabilidad por tipo de accidente
prob_tipo <- dt_modelo %>%
...
```{r}
# Probabilidad por franja del día
